...
std::pair<T, T> operator+(const std::pair<T, T>& lhs, U value)
{
    ...
    if (result.second <= (T)(std::numeric_limits<T>::max() - (T)value))
    {
        result.second += value;
    }
    else
    {
        result.first += 1;
        result.second = (T)value - (T)(std::numeric_limits<T>::max() - result.second);
    }
    return result;
}
...
template<typename T1, typename T2, typename U, std::enable_if_t<
    ...
    std::is_convertible<U, T1>::value>* = nullptr>
